    ...
    for line_no, line in enumerate(text.splitlines(), start=1):
        for severity, pattern in SEVERITY_RULES:
            if pattern.search(line):
                ...
                if len(findings) < 500:
                    findings.append(
                        {"line": line_no, "severity": severity, "message": line.strip()[:240]}
                    )
                break
...
def load_diagnosis(name: str, size: int, raw: bytes) -> list[dict] | None:
    ...
    for raw_item in items:
        ...
        epoch_end = raw_item.get("epoch_end") or raw_item.get("end_epoch") or epoch_start
        ...
        if fix is None or fix == "null":
            fix = ""
        fix_status = raw_item.get("fix_status") or "hypothesis — verify in repo"
...
def render_anomaly_chart(
    ...
    # Partition zones into the panel they should shade.
    LR_ZONE_TYPES = {"lr", "lr_stall", "stalled_lr"}
    loss_zones = [z for z in zones if z["type"].lower() not in LR_ZONE_TYPES]
    lr_zones = [z for z in zones if z["type"].lower() in LR_ZONE_TYPES]
    ...
    def build_panel(
        ...
        if use_log:
            ...
            import numpy as np
            long = long[(long["value"] > 0) & np.isfinite(long["value"])]
        ...
        for i, z in enumerate(panel_zones):
            r = _zone_to_x(z, df, xcol)
            if r is None:
                continue
            x0, x1 = r
            mid = (x0 + x1) / 2.0
            sev_color = SEVERITY_COLORS.get(z["severity"], "#64748b")
            # Stagger label heights to avoid overlap when zones are adjacent
            y_frac = 0.92 if i % 2 == 0 else 0.75
            row = pd.DataFrame([{
                "x0": x0, "x1": x1, "mid": mid,
                "ytop": y_top * y_frac,
                "label": z["type"],
    ...
    spike_zones = [z for z in zones if "spike" in z["type"].lower()]
    if spike_zones and loss_metrics and "epoch" in df.columns:
        z = spike_zones[0]
        ...
        with st.expander(
            ...
            if not long.empty:
                ...
                peak_iter = (
                    z.get("iteration_start")
                    or int((z["epoch_start"] - 1) * (len(df) / df["epoch"].nunique()) + 25)
                )
                st.caption(
...
def render_diagnosis_cards(zones: list[dict]) -> None:
    ...
    for z in zones:
        ...
        with st.container(border=True):
            ...
            # Suggested fix with fix_status badge
            fix = z.get("suggested_fix") or ""
            fix_status = z.get("fix_status") or ""
            if fix and fix != "null":
                # Green badge for confirmed, yellow for hypothesis
                if "confirmed" in fix_status.lower():
                    badge = "🟢 **Confirmed in repo**"
                    st.success(f"{badge}\n\n**Fix:** {fix}")
                elif "hypothesis" in fix_status.lower() or "verify" in fix_status.lower():
                    badge = "🟡 **Hypothesis**"
                    st.warning(f"{badge}\n\n**Fix:** {fix}")
                else:
                    st.info(f"**Suggested fix:** {fix}")
            elif fix_status and "not applicable" not in fix_status.lower():
                st.caption(f"Fix status: {fix_status}")
...
def render_logs_panel(intake: Intake) -> None:
    if not intake.logs:
    ...
    # Findings table
    if findings:
        st.dataframe(pd.DataFrame(findings), hide_index=True)
    else:
...
def render_tables_panel(intake: Intake) -> None:
    if not intake.data:
        st.info("No .csv / .tsv files in this intake. Drop a metrics export to plot curves.")
        return
    ...
    diag_file = st.file_uploader(
        "🧠 Attach diagnosis file (.json) — from Bob / analysis pipeline",
        type=["json"],
        key="dbg_diag",
    )
    zones = None
    if diag_file:
        try:
            zones = load_diagnosis(diag_file.name, diag_file.size, diag_file.getvalue())
        except Exception:
            zones = None
        if zones is None:
            st.warning(
    ...
    if not nan_cols.empty:
        st.warning(f"🕳️ NaN audit — {int(nan_cols.sum()):,} missing value(s): "
                   + ", ".join(f"`{c}` ({v})" for c, v in nan_cols.items()))
    else:
        st.caption("NaN audit: table is fully populated.")

    # Diagnosis cards (rendered below the chart + NaN audit so the visual story reads top-down)
    if zones:
        render_diagnosis_cards(zones)


def render_debugger(intake: Intake) -> None:
    render_intake_metrics(intake)
    logs_tab, tables_tab = st.tabs(["🩺 Training logs", "📈 Metric tables"])
    with logs_tab:
        render_logs_panel(intake)
    with tables_tab:
        render_tables_panel(intake)
